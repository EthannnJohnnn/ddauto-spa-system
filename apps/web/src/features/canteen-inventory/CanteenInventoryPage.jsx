...
import {
  createCanteenDocument,
  createCanteenProduct,
  getCanteenInventoryOverview,
  setCanteenDocumentActive,
  setCanteenProductActive,
  updateCanteenDocument,
  updateCanteenProduct,
} from './canteen-inventory-api.js';
...
export function CanteenInventoryPage({ csrfToken }) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setOverview(await getCanteenInventoryOverview(period.start, period.end));
    } catch (loadError) {
  ...
  async function saveProduct(productId, values) {
    if (productId) await updateCanteenProduct(productId, values, csrfToken);
    else await createCanteenProduct(values, csrfToken);
    await load();
  }

  async function saveDocument(documentId, values) {
    if (documentId) await updateCanteenDocument(documentId, values, csrfToken);
    else await createCanteenDocument(values, csrfToken);
  ...
  async function confirmStatus(reason) {
    if (statusTarget.kind === 'product') {
      await setCanteenProductActive(statusTarget.item.id, statusTarget.isActive, reason, csrfToken);
    } else {
  ...
  if (loading && !overview) return <PageMessage title="Loading canteen inventory…" />;
  if (!overview) {
    return <PageMessage detail={error} onRetry={load} title="Canteen inventory could not load" />;
  }
          ...
          <div>
            <div className="mb-4 flex flex-wrap gap-2">
              {stockTypes.map(([value, label]) => (
                <button
                  className={`rounded-xl px-4 py-2.5 text-sm font-semibold ${stockType === value ? 'bg-slate-900 text-white' : 'border border-slate-300 bg-white text-slate-600'}`}
  ...
  const cards = [
    {
      ...
      note: `${summary.canteenUnitsSold} units sold`,
    },
    {
