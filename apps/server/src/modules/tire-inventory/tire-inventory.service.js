...
export class TireInventoryService {
  ...
  getOverview({ start, end }) {
    ...
    return {
      ...
      documents,
      summary: {
        tireSalesCentavos: sum(saleItems.map((item) => item.lineTotalCentavos)),
        tireUnitsSold: sum(saleItems.map((item) => item.quantity)),
  ...
  createProduct(input, actorUserId) {
    ...
    this.repository.transaction(() => {
      ...
      this.auditRepository.record({
        actorUserId,
        action: 'TIRE_PRODUCT_CREATED',
        entityType: 'TIRE_PRODUCT',
  ...
  updateProduct(productId, input, actorUserId) {
    ...
    const updated = {
      ...
      tireType: input.tireType ?? current.tire_type,
    ...
    this.assertProductNameAvailable(updated.name, updated.size, productId);
    const now = this.now();
    this.repository.transaction(() => {
      this.repository.updateProduct(productId, { ...updated, now });
      this.auditRepository.record({
        actorUserId,
        action: 'TIRE_PRODUCT_UPDATED',
        entityType: 'TIRE_PRODUCT',
        entityId: String(productId),
  ...
  setProductActive(productId, isActive, reason, actorUserId) {
    ...
    this.repository.transaction(() => {
      ...
      this.auditRepository.record({
        actorUserId,
        action: isActive ? 'TIRE_PRODUCT_RESTORED' : 'TIRE_PRODUCT_ARCHIVED',
        entityType: 'TIRE_PRODUCT',
        entityId: String(productId),
        metadata: { reason },
        now,
  ...
  createDocument(input, actorUserId) {
    ...
    this.repository.transaction(() => {
      ...
      this.auditRepository.record({
        actorUserId,
        action: 'TIRE_DOCUMENT_CREATED',
        entityType: 'TIRE_INVENTORY_DOCUMENT',
  ...
  setDocumentActive(documentId, isActive, reason, actorUserId) {
    ...
    if (current.status === targetStatus) {
      throw new AppError(
        409,
        'TIRE_DOCUMENT_STATUS_UNCHANGED',
    ...
    this.repository.transaction(() => {
      ...
      this.auditRepository.record({
        actorUserId,
        action: isActive ? 'TIRE_DOCUMENT_RESTORED' : 'TIRE_DOCUMENT_VOIDED',
        entityType: 'TIRE_INVENTORY_DOCUMENT',
        entityId: String(documentId),
        metadata: { reason },
        now,
  ...
  requireDocument(documentId) {
    ...
    if (!document) {
      throw new AppError(404, 'TIRE_DOCUMENT_NOT_FOUND', 'The tire document was not found.');
    }
    return document;
  }
  ...
  requireProduct(productId) {
    const product = this.repository.findProduct(productId);
    if (!product) {
      throw new AppError(404, 'TIRE_PRODUCT_NOT_FOUND', 'The tire product was not found.');
    }
    return product;
  }
  ...
  assertProductNameAvailable(name, size, excludeId = 0) {
    if (this.repository.findProductByNameAndSize(name, size, excludeId)) {
      throw new AppError(
        409,
        'TIRE_PRODUCT_DUPLICATE',
        'A tire product with that name and size already exists.',
      );
    }
  }
  ...
  assertStockNonnegative(productIds) {
    ...
    for (const row of this.repository.listDailyStockDeltas([...new Set(productIds)])) {
      ...
      if (running < 0) {
        const label = row.size ? `${row.name} (${row.size})` : row.name;
        throw new AppError(
          409,
          'INSUFFICIENT_TIRE_STOCK',
          `${label} would have negative stock on ${row.business_date}.`,
        );
      }
...
function normalizeDocumentItem({ documentType, item, product }) {
  ...
  return {
    ...
    tireTypeSnapshot: product.tire_type,
...
function mapDocument(row, itemRows) {
  const items = itemRows.map((item) => ({
    ...
    tireType: item.tire_type_snapshot,
